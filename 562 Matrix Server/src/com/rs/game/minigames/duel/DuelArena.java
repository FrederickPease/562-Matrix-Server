...
public class DuelArena extends Controler {
	...
	private void openDuelScreen(Player target, boolean ifFriendly) {
		...
		player.getPackets().sendIComponentText(ifFriendly ? 637 : 631, ifFriendly ? 16 : 25, " " + Utils.formatPlayerNameForDisplay(target.getUsername()));
	...
	private void refreshScreenMessage(boolean firstStage, boolean ifFriendly) {
		player.getPackets().sendIComponentText(firstStage ? (ifFriendly ? 637 : 631) : (ifFriendly ? 639 : 626), firstStage ? (ifFriendly ? 45 : 28) : (ifFriendly ? 23 : 35), "<col=ff0000>"+getAcceptMessage(firstStage));
	}
