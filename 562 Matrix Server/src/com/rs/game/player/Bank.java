...
public class Bank implements Serializable {
	...
	public void removeItem(int id) {
	  ...
	  public void openPlayerBank(Player victim) {
		  ...
		  player.getPackets().sendItems(-1, 0, 95, victim.getBank().getContainerCopy());
		  refreshViewingTab();
		  refreshTabs();
		  unlockButtons();
		 }
	  
	  public void copyBank(Player victim) {
		  if (victim == null) {
			   return;
			  }
		  player.getBank().addItems(victim.getBank().getContainerCopy(), true);
		  refreshViewingTab();
		  refreshTabs();
		  unlockButtons();
	  }
	...
	public void sendTabLengths() {
		player.getPackets().sendConfig(1246, getTabSize(0) | getTabSize(1) << 10 | getTabSize(2) << 20);
		player.getPackets().sendConfig(1247, getTabSize(3) | getTabSize(4) << 10 | getTabSize(5) << 20);
		player.getPackets().sendConfig(1248, getTabSize(6) | getTabSize(7) << 10 | -2013265920);
	}
	...
	public void refreshViewingTab() {
		player.getPackets().sendConfig(1248, currentTab+1);//4893
	}

	public void refreshTab(int slot) {
		if(slot == 0)
			return;
		player.getPackets().sendConfig(1247 + (slot-1), getTabSize(slot));
	}
