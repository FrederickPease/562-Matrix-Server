...
public class ClanChat {
    ...
    public ClanChat(String owner, String name) {
        this.roomName = name;
    ...
    public Byte getRank(Player player) {
        if (Utils.formatPlayerNameForProtocol(player.getUsername()).equals(roomOwner)) {
            return 7;
    ...
    public void toggleLootshare() {
        ...
        for (Player pl : members) {
        	//player.getPackets().sendGameMessage(pl""4");
            player.getPackets().sendConfig(1083, lootsharing ? 1 : 0);
        }
    }
