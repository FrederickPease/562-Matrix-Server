...
public class GnomeAgility {
	...
	public static void swimriver(final Player player) {
		if (player.getX() != 2512 || player.getY() != 3476
				|| player.getPlane() != 0)
			return;
		final boolean running = player.getRun();
		player.setRun(false);
		player.lock(7);
		player.addWalkSteps(2512, 3467, -1, false);
		WorldTasksManager.schedule(new WorldTask() {
			boolean secondloop;
			...
			public void run() {
				if (!secondloop) {
					secondloop = true;
					player.getAppearence().setRenderEmote(189);//191 carpet//294 for wa3ad
					//player.setNextAnimation(new Animation(766));
				} else {
					player.getAppearence().setRenderEmote(-1);
					player.setRun(running);
					player.getSkills().addXp(Skills.AGILITY, 7);
					player.getPackets().sendGameMessage(
							"You swim succesfully.", true);
					stop();
				}
			}
		}, 0, 5);
