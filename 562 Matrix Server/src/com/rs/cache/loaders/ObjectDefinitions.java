...
public class ObjectDefinitions {
    ...
    private static ConcurrentHashMap<Integer, ObjectDefinitions> objectDefinitions = new ConcurrentHashMap<Integer, ObjectDefinitions>();
	...
	private void method3297(InputStream stream) {
    ...
    public static ObjectDefinitions getObjectDefinitions(int id) {
        ...
        if(def == null) {
        	...
        	def.id = id;
        	byte[] data = Cache.getCacheFileManagers()[16].getFileData(getContainerId(id), id & 0xff);
    		if (data == null) 
    			System.out.println("Failed loading Object " + id+".");
    		else
    			def.readValueLoop(new InputStream(data));
            def.method3287();
    		if ((def.name != null && (def.id == 26972 || def.name
			...
			if (def.clipType == 0)
				def.clipType = 1;
		} else if (Clipedobject.Counter(id)) {
			def.notCliped = false;
			def.projectileCliped = true;
			if (def.clipType == 0)
				def.clipType = 1;
